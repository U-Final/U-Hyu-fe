...
export default function TotalStatsCard({ data }: TotalStatsCardProps) {
  return (
    <div className="grid grid-cols-2 gap-4 w-full mb-6">
      ...
      <div className="p-4 rounded-lg shadow bg-white flex flex-col items-center">
        <span className="text-sm text-gray-500 mb-1">검색</span>
        <span className="text-2xl font-bold text-primary">{data.totalSearch}</span>
      </div>
      <div className="p-4 rounded-lg shadow bg-white flex flex-col items-center">
        <span className="text-sm text-gray-500 mb-1">멤버십</span>
        <span className="text-2xl font-bold text-primary">{data.totalMembership}</span>
      </div>
    </div>
  );
} 