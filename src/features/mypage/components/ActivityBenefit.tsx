...
import { ActivityBenefitDetailSkeleton } from '@/shared/components/skeleton';
...
const ActivityBenefit = () => {
  ...
  if (isLoading) {
    return <ActivityBenefitDetailSkeleton />;
  }
      ...
      {!data.discountMoney || data.discountMoney === 0 ? (
        <div className="flex flex-col items-center justify-center py-8">
          ...
          <p className="text-sm text-gray-500 text-center max-w-md">
            {userData?.data?.nickName || '사용자'}님의{' '}
            {userData?.data?.grade || '등급'} 등급으로 다양한 혜택을 받아보세요!
          </p>
        </div>
      ) : (
        <div className="relative">
          <div className="absolute inset-0 overflow-hidden rounded-xl">
            <div
              className="absolute top-4 left-4 w-2 h-2 bg-yellow-star rounded-full animate-bounce"
              style={{ animationDelay: '0s' }}
            ></div>
            <div
              className="absolute top-6 right-8 w-1.5 h-1.5 bg-primary rounded-full animate-bounce"
              style={{ animationDelay: '0.3s' }}
            ></div>
            <div
              className="absolute top-2 right-4 w-1 h-1 bg-blue rounded-full animate-bounce"
              style={{ animationDelay: '0.6s' }}
            ></div>
            <div
              className="absolute top-8 left-8 w-1.5 h-1.5 bg-green-star rounded-full animate-bounce"
            ...
            ></div>
            <div
              className="absolute top-5 left-2 w-1 h-1 bg-orange-star rounded-full animate-bounce"
          ...
          <div className="relative bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl p-6 shadow-sm border border-gray-200">
            <div className="text-center mb-6">
              ...
              <p className="text-sm text-gray-600">
                {userData?.data?.nickName || '사용자'}님, 지금까지 이만큼
                절약하셨네요
              </p>
            </div>
