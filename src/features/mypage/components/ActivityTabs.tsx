...
const ActivityTabs = ({ activeTab, setActiveTab }: Props) => {
  const tabs = ['활동 내역', '즐겨찾기'];

  return (
    <div className="sticky top-0 z-10 pt-[1rem] pb-[0.5rem]">
      <div className="relative flex items-center justify-between bg-gray-100 rounded-[1rem] px-[0.25rem] h-[3rem]">
        <div
            className={`
                absolute top-[0.35rem] bottom-[0.35rem] left-[0.25rem]
                w-[calc(50%-0.37rem)]   // 좌측 패딩 보정
                rounded-[0.8rem] bg-blue-500
                transition-transform duration-300 ease-in-out
                ${activeTab === '활동 내역' ? 'translate-x-0' : 'translate-x-[calc(100%+0.25rem)]'}
            `}
        />
